...
export class TablaSimbolos {
    ...
    private hijos: TablaSimbolos[]

    constructor(anterior?: TablaSimbolos, nombreEntorno: string = "Global") {
        this.tabla = new Map()
        this.anterior = anterior
        this.nombreEntorno = nombreEntorno
        this.hijos = []
        if (anterior) {
            anterior.agregarHijo(this)
        }
    }
    ...
    private agregarHijo(hijo: TablaSimbolos): void {
    ...
    getAllSymbols(): Simbolo[] {
        const actuales = Array.from(this.tabla.values())
        const descendientes = this.hijos.flatMap((hijo) => hijo.getAllSymbols())
        return [...actuales, ...descendientes]
    }
